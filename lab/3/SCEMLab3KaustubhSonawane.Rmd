...
-   **DO NOT** add calls to `install.packages()` into your solutions.
    ...
    listed in the "setup" code block below.
...
change in it is to replace `ABCDEF` by your student ID number, which
...
Example of using the R function `optim()` to estimate the MLE values for
the mean and variance of a normal distribution. *You don't need to
change anything in the code block below.*
...
llik.cauchy = function(par, x){
  theta = par[1]
  n = length(n)
  llik_var = -n * log(pi) - sum(log(1 + (x - theta)^2))  # Doubt here? I dont think I am summing it 10000 times. Maybe a loop was needed?
  return(llik_var)
}

x <- rcauchy(n, location = theta)  # this initialization is the correct one

# Initial guess of theta 1
init = c(theta = 1)

# My method which is a bit lacking
# fit = optim(
#   par = init, fn = llik.cauchy, x = x,
#   method = 'L-BFGS-B'
# )


#  Prof's suggested method
fit = optim(
  par = init, fn = llik.cauchy, x = x,
  method = 'L-BFGS-B',
  control = list(fnscale = -1)  # This is the most important parameter to make it a maximisation problem
)
...
# Prof's approach
cauchy.vector = numeric(n_samples)
for (i in 1:n_samples){
...
ggplot(data = data.frame(cauchy.vector), aes(x = cauchy.vector)) +
  geom_density() +
  theme_minimal() +
...
ggplot(data = delayed_flights_2h, aes(x = dep_delay, y = origin, fill=origin)) + geom_violin()
...
ggplot(slice_sample(flights, prop = .1), aes(x = dep_delay, y = arr_delay)) +
  geom_point() +
...
Based on the plot, The arrival delay and departure delay are directly
correlated. i.e. if the flight is late for arrival, then it is late for
departure and vise versa.
...
corr_delayed <- corr_delayed %>%
  mutate(cor = paste0("corr = "), signif(3))

ggplot(slice_sample(flights, prop = .20), aes(x = dep_delay, y = arr_delay)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(~origin) +
  geom_text(data = corr_delayed, aes(label = cor), x = 250, y = 1000, size = 3)
