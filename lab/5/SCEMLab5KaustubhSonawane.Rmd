...
author: "Kaustubh Heamnt Sonawane (2701553)"
...
```{r Q1b}
## Question 1.b
ggplot(data = df, aes(x = df$Estimate)) + geom_histogram(bins = 10)
...
ggplot(data = df2, aes(x = df2$Estimate)) + geom_histogram(bins = 10)
...
$$
\begin{cases}
H_0: add~your~null~hypothesis\\
H_1: add~your~alternative~hypothesis
\end{cases}
$$
:::
...
ggplot(df2, aes(x = Estimate)) +
  ...
  annotate("pointrange", y = .005, x = mytest$estimate, xmin = mytest$conf.int[1], xmax = mytest$conf.int[2]) +
  # Add p-value
  annotate("text", x = mytest$estimate, y = 0.01, label = paste0("p = ", signif(mytest$p.value, 2))) +
  # Add text annotation
...
P-value for PPV is 5.17e-13 which is significantly lower that 0.01 and true mean 0.683 is also greater than 0.5 thus null hypothesis rejected.

P-value for NPV is 0.9997 which is significantly higher that 0.01 and true mean 0.402 is also lower than 0.5 thus null hypothesis is accepted.
...
Sine the null hypothesis was not rejected for NPV even though it was rejected for PPV the baseline model is not better.
:::
...
myN = power.t.test(
  ...
  power = 0.9,
  sig.level = 0.05,
  type = "two.sample",  # Not sure as to what should be here? two since it is an improvement being tested?
...
power_calc = function(n){
  power_var = power.t.test(
    n = n,
    delta = 0.01,
    sd = 0.05,
    # power = 0.9,  # Not Needed
    sig.level = 0.05,
    type = "two.sample",  # Not sure as to what should be here? two since it is an improvement being tested?
    alternative = "one.sided",
  )$power
...
ggplot(data = power.df, aes(x = power.df$n, y = power.df$power)) +
  geom_line() +
  xlab('N') +
  ylab('Power')
...
df.upm = readRDS("UPMSP_SA_full_results.rds")
...
q5.df.summ = df.upm1 %>%
  group_by(Instance) %>%
  summarise(
    Mean.MS = mean(Makespan)
  )

dim(q5.df.summ)  # 57 Rows as expected
```
...
$$
\begin{cases}
H_0: \mu_{makespan} >= 240\\
H_1: \mu_{makespan} < 240
\end{cases}
...
## Don't forget to store the resulting summarized data frame as variable q5.t.power

q5.t.power = power.t.test(
  n = 57,
  delta = 0.5,
  sd = 1,
  sig.level = 0.05,
  type = "one.sample",  # Not sure as to what should be here? two since it is an improvement being tested?
  alternative = "one.sided",  # one sided since we are chaking if mean is lower?
)$power
...
# Still unsure as to how to read the below graph
ggplot(data = q5.df.summ, aes(sample = Mean.MS)) +
